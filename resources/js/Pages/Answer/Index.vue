...
export default defineComponent({
    ...
    data() {
        return {
            ...
            isModalOpen: false,
            activeId: "",
            tbody: ["Question", "Answer", "Order By", "Action"],
        };
    },
    components: {
        AppLayout,
...
<template>
    <app-layout :title="title">
        <template #breadcrumb>
            ...
            <li class="breadcrumb-item">
                <Link href="/answers" class="text-muted text-hover-primary">{{
                    title
                }}</Link>
            </li>
        </template>
        ...
        <AnswerForm
            :show="isModalOpen"
            @hidemodal="hideAnswerForm"
            :id="activeId"
            :questions="questions"
        />
        <div class="card">
            <form
                class="card-header justify-content-start p-5 gap-3"
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table
                        class="table align-middle table-row-dashed fs-6 gy-5 text-left"
                    >
                        <thead>
                            <tr
                                class="text-gray-700 fw-bold fs-7 text-uppercase"
                            >
                        ...
                        >
                            <tr
                                v-for="(answer, index) in answers.data"
                                :key="index"
                            >
            ...
            <div class="card-footer">
                <div class="row" v-if="answers.data.length > 0">
                    <div
                        class="col-sm-12 d-flex align-items-center justify-content-between"
                        v-if="answers.meta"
                    >
                        <span class="fw-bold text-gray-700">
                            Showing {{ answers.meta.from }} to
                            {{ answers.meta.to }} of
                            {{ answers.meta.total }} entries
                        </span>
