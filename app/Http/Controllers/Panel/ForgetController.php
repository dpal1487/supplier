...
class ForgetController extends Controller
{
    ...
    public function verifyOtp(Request $request)
    {
        ...
        {
            if(User::where(['email'=>$request->email,'reset_otp'=>$request->otp_number])->first())
            {
    ...
    public function passwordReset($request){
        ...
        {  
            $status = User::where(['email'=>$request->email,'reset_otp'=>$request->otp_number])->update(['password'=>Hash::make($request->password), 'reset_otp'=>0]);
                if($status)
                {
